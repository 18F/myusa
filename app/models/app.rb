...
#  has_many :app_activity_logs
  ...
  validates_uniqueness_of :slug, scope: :deleted_at, message: proc { |_a, b|
    "\"#{b[:value]}\" has already been taken"
  }
...
#  attr_accessible :user, :user_id, :is_public, as: :admin
  ...
  has_attached_file(:logo,
                    ...
                    default_url: '/assets/app-icon.png')
  ...
  def can_delete?
    !self.is_public? && app_activity_logs.blank?
  end
  ...
  class << self
    ...
    def authentic_apps
      not_sandbox.where("name != 'Default App'")
    end
  end
  ...
  def redirect_uri
    if @redirect_uri.blank?
      (oauth2_client && oauth2_client.redirect_uri)
    else
      @redirect_uri
    end
  end
  ...
  def client_id
    oauth2_client && oauth2_client.client_id
  end

  def oauth2_client
    @oauth2_client || oauth2_clients.first
  end
  ...
  def find_scopes(scopes = nil)
    ...
    oauth_scopes.where('oauth_scopes.scope_name' => scopes)
  end

  def self.client_id_from_uri(uri)
    uri.match(/[\?&;]client_id=([^&;]+)/).try(:[], 1)
  end

  def self.find_by_return_to_url(return_to_url)
    return nil unless return_to_url
    client_id = client_id_from_uri(return_to_url)
  ...
  def create_oauth2_client
    @oauth2_client = Songkick::OAuth2::Model::Client.new(name: name,
