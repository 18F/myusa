...
[nil, 1, 2].each do |version_number| 
  ...
  describe "API" do
    ...
    before do
      Rack::MockRequest::DEFAULT_ENV["HTTP_ACCEPT"] = "application/vnd.myusa.v#{version_number}" if version_number
    ...
    after do
      # Avoid error: Missing template oauth/authorize, application/authorize with {:formats=>["application/vnd.myusa.v1"]}
    ...
    describe "Tasks API" do
      before do
        #Rack::MockRequest::DEFAULT_ENV["HTTP_ACCEPT"] =  "application/vnd.myusa.v2"
        @token = build_access_token(@app)
      end
    ...
    describe "Authorized Scopes API" do
      describe "GET /api/authorized_scopes" do
        context "when a valid token is provided" do
          ...
          if version_number && version_number > 1 # For version 2 only
